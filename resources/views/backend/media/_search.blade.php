<div class="card mb-3">
    <div class="card-body">
        <form id="media-filter-form" method="GET" action="{{ route('media.index') }}" class="row g-2">
            {{-- Title Filter --}}
            <div class="col-md-3">
            ...
            <div class="col-md-3">
                <select name="filter[type]" id="filter-type" class="form-control">
                    ...
                    <option value="image" {{ request('filter.type') == 'image' ? 'selected' : '' }}>Image</option>
                    <option value="pdf" {{ request('filter.type') == 'pdf' ? 'selected' : '' }}>PDF</option>
                </select>
            </div>
